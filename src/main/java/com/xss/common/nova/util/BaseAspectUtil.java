...
@Slf4j
public class BaseAspectUtil {
    private static final Set<Class> jsonIgnoredClasses = BaseJsonUtil.jsonIgnoredClasses;
    ...
    static {
        ...
        mapper.registerModule(BaseJsonUtil.getSimpleModule());
    }
            ...
            IntStream.range(0, args.length).forEach(index -> {
                if (!(args[index] instanceof InputStream)) {
                    ...
                    if (args[index] != null && params[index].getAnnotation(LogIgnore.class) == null) {
                        if (BaseObjectUtil.isPrimitive(args[index].getClass()) && params[index].getAnnotation(JsonMosaic.class) != null) {
                            JsonMosaic jsonMosaic = params[index].getAnnotation(JsonMosaic.class);
                            sParam = BaseJsonUtil.writeValue(args[index]);
                            sParam = BaseStringUtil.mosaic(sParam, jsonMosaic, '*');
                        } else {
            ...
            IntStream.range(0, args.length).forEach(index -> {
                try {
                    ...
                    if (args[index] != null && params[index].getAnnotation(LogIgnore.class) == null) {
                        if (BaseObjectUtil.isPrimitive(args[index].getClass()) && params[index].getAnnotation(JsonMosaic.class) != null) {
                            JsonMosaic jsonMosaic = params[index].getAnnotation(JsonMosaic.class);
                            String text = BaseJsonUtil.writeValue(args[index]);
    ...
    private static Map<String, String> toMap(String name, Object value, int crtDepth) {
        ...
        if (value != null && crtDepth < 5) {
            if (BaseObjectUtil.isPrimitive(value.getClass())) {
                    ...
                    if (fields != null) {
                        Stream.of(fields).forEach(field -> {
                            ...
                            if (fieldValue != null && field.getAnnotation(LogIgnore.class) == null) {
                                if (BaseObjectUtil.isPrimitive(fieldValue.getClass()) && field.getAnnotation(JsonMosaic.class) != null) {
                                    JsonMosaic jsonMosaic = field.getAnnotation(JsonMosaic.class);
                                    String text = BaseJsonUtil.writeValue(fieldValue);
                                    fieldValue = BaseStringUtil.mosaic(text, jsonMosaic, '*');
                                }
