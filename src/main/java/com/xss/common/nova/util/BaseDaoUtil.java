...
public class BaseDaoUtil {
    ...
    static {
        URL baseUrl = BaseDaoUtil.class.getClassLoader().getResource(".");
        try {
    ...
    public static void generateCode(DbInfo dbInfo, List<String> tables, String basePackage, boolean override) throws Exception {
        ...
        try {
            ...
            schema = BaseStringUtil.underScoreToCamel(schema, false);
            ...
            if (BaseCollectionUtil.isEmpty(tables)) {
    ...
    private static void generateCode4Table(String schema, DatabaseMetaData meta, String table, String basePackage, boolean override) {
        try {
            ...
            if (rs != null) {
                ...
                if (BaseCollectionUtil.isNotEmpty(errorList)) {
                ...
                while (primaryKeyRs.next()) {
                    ...
                    BaseCollectionUtil.add(primaryKeys, primaryKeyRs.getShort("KEY_SEQ") - 1, primaryKeyRs.getString("COLUMN_NAME"));
                }
                ...
                while (indexInfoRs.next()) {
                    ...
                    if (unique && (uniqueCols == null || indexName.equalsIgnoreCase(uniqueIndexName))) {
                        ...
                        BaseCollectionUtil.add(uniqueCols, indexInfoRs.getShort("ORDINAL_POSITION") - 1, indexInfoRs.getString("COLUMN_NAME"));
                    }
                    ...
                    if (!unique && (indexCols == null || indexName.equalsIgnoreCase(otherIndexName))) {
                        ...
                        BaseCollectionUtil.add(indexCols, indexInfoRs.getShort("ORDINAL_POSITION") - 1, indexInfoRs.getString("COLUMN_NAME"));
                    }
                }
    ...
    private static String entityContent(String table, String basePackage, String entityName, Map<String, String> nameTypes, Map<String, String> remarks) {
        ...
        nameTypes.entrySet().forEach(entry -> {
            ...
            if (BaseCollectionUtil.contains(primaryKeys, entry.getKey())) {
                buf.append("    @PrimaryKey\n");
                containAnno.set(true);
            } else if (BaseCollectionUtil.contains(uniqueKeys, entry.getKey())) {
                buf.append("    @UniqueIndex\n");
                containAnno.set(true);
            } else if (BaseCollectionUtil.containsIgnoreCase(UPDATE_IGNORE_COLS, entry.getKey())) {
    ...
    private static String daoContent(String schema, String basePackage, String entityName, String table, Map<String, String> nameTypes) {
        ...
        buf.append("import com.xss.common.nova.util.BaseJdbcUtil;").append("\n");
        ...
        buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getInsert(getTable(), ").append(ensure(entityCamelName)).append(", dbMapping);\n");
        ...
        if (primaryKeysMap.containsKey(table)) {
            ...
            buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getInsertIgnore(getTable(), ").append(ensure(entityCamelName)).append(", dbMapping);\n");
            buf.append("        return template.update(jdbcResult.getSql(), jdbcResult.getParams()) == 1;\n");
            buf.append("    }\n\n");
            ...
            buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getInsertOrUpdate(getTable(), ").append(ensure(entityCamelName)).append(", dbMapping);\n");
            buf.append("        return template.update(jdbcResult.getSql(), jdbcResult.getParams()) == 1;\n");
            buf.append("    }\n\n");
        }
        ...
        buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getBatchInsert(getTable(), ").append(entityCamelName).append("s, dbMapping);\n");
        ...
        buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getUpdate(getTable(), ").append(ensure(entityCamelName)).append(", dbMapping, ")
        ...
        buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getPatch(getTable(), ").append(ensure(entityCamelName)).append(", dbMapping, ")
                .append(primaryKeySimpleArgs(table, nameTypes)).append(");\n");
        buf.append("        return template.update(jdbcResult.getSql(), jdbcResult.getParams()) == 1;\n");
        buf.append("    }\n\n");

        //get by primary key or unique key
        if (BaseCollectionUtil.isNotEmpty(primaryKeysMap.get(table))) {
            buf.append(getByCols(primaryKeysMap.get(table), nameTypes, entityName, true));
        }

        if (BaseCollectionUtil.isNotEmpty(uniqueIndexMap.get(table))) {
        ...
        if (indexMap.containsKey(table) ||
            ...
            buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getSelect(getTable(), ");
            ...
            buf.append("                ").append(".map(dbRow -> BaseJdbcUtil.dbRowToPo(dbRow, dbMapping, ").append(entityName).append("Po.class))\n");
        ...
        buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getSelectForCount(getTable(), (Criteria) null);\n");
        ...
        buf.append("        }\n\n");
        buf.append("        jdbcResult = BaseJdbcUtil.getSelect(getTable(), (Criteria) null, pageRequest);\n");
        buf.append("        List<").append(entityName).append("Po> datas = template.queryForList(jdbcResult.getSql(), jdbcResult.getParams()).stream()\n");
        buf.append("                .map(dbRow -> BaseJdbcUtil.dbRowToPo(dbRow, dbMapping, ").append(entityName).append("Po.class))\n");
        ...
        buf.append("        JdbcResult jdbcResult = BaseJdbcUtil.getDelete(getTable(), ").append(criteriaArgs(16, keyCols)).append(");\n");
    ...
    private static String getByCols(List<String> keyCols, Map<String, String> nameTypes, String entityName, boolean primaryKey) {
        ...
        buf.append("            return BaseJdbcUtil.dbRowToPo(dbRow, dbMapping, ").append(entityName).append("Po.class);\n");
    ...
    private static String toCamel(String src, boolean firstUpper) {
        return BaseStringUtil.underScoreToCamel(src, firstUpper);
    }
    ...
    private static String criteriaArgs(int space, List<String> cols) {
        StringBuilder buf = new StringBuilder();
        if (BaseCollectionUtil.isEmpty(cols)) {
    ...
    private static List<String> primaryKeys(String table, Map<String, String> nameTypes) {
        ...
        } else if (BaseCollectionUtil.isNotEmpty(uniqueIndexMap.get(table))) {
            return uniqueIndexMap.get(table);
        } else if (BaseCollectionUtil.isNotEmpty(indexMap.get(table))) {
