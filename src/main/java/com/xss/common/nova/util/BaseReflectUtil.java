...
public class BaseReflectUtil {
    public static Object parse(String src, Type type) {
        ...
        if (isArray) {
            Stream stream = BaseJsonUtil.readValues(src, String.class).stream()
                    .map(innerSrc -> BaseReflectUtil.parse(innerSrc, cls));
        ...
        if (cls == Date.class || cls == java.sql.Date.class) {
            return BaseDateUtil.parseDate(src);
        }

        try {
            return BaseJsonUtil.readValue(src, cls);
        } catch (Throwable e) {
