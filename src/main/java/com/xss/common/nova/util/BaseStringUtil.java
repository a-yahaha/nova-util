...
public class BaseStringUtil {
    ...
    private static final Date START_DATE = BaseDateUtil.fromDateFormat("2010-01-01");
    ...
    public static String mergeSave(String s1, String s2, String ignoredChars) {
        ...
        if (length(s1, ignoredChars) != 0) {
            if (length(s2, ignoredChars) != 0) {
                String merged = BaseStringUtil.merge(s1, s2, "*#");
    ...
    public static String timeUUID(Integer randomLen, String prefix, String suffix) {
        ...
        buf.append(BaseDateUtil.format(new Date(), "yyMMddHHmmssSSS")).append(machineId(3)).append(innerRandom(UidType.TIME, randomLen));
    ...
    public static long longUid(String... args) {
        ...
        String argsHash = BaseSecurityUtil.md5(true, args);
    ...
    public static String toCamel(String src, String separators, Boolean firstLetterUpperCase) {
        ...
        if (firstLetterUpperCase != null) {
            if (firstLetterUpperCase) {
                src = BaseStringUtil.convertFirstLetter(src, true);
            } else {
                src = BaseStringUtil.convertFirstLetter(src, false);
            }
        }
        return src;
    }
    ...
    public static String convertCamel(String src, String separator, Boolean firstLetterUpperCase) { //userEntity -> user-entity
        ...
        if (firstLetterUpperCase != null) {
            if (firstLetterUpperCase) {
                result = BaseStringUtil.convertFirstLetter(result, true);
            } else {
                result = BaseStringUtil.convertFirstLetter(result, false);
            }
        }
    ...
    public static String mosaic(String text, JsonMosaic mosaic, char mosaicChar) {
        ...
        if (mosaic.end() != 0) {
            if (mosaic.end() < 0) {
                text = BaseStringUtil.mosaic(text, mosaic.start(), text.length() + mosaic.end(), mosaicChar);
            } else {
                text = BaseStringUtil.mosaic(text, mosaic.start(), mosaic.end(), mosaicChar);
            }
        } else if (mosaic.length() != 0) {
            text = BaseStringUtil.mosaic(text, mosaic.start(), mosaic.start() + mosaic.length(), mosaicChar);
        } else {
            text = BaseStringUtil.mosaic(text, mosaic.start(), mosaic.start() + text.length(), mosaicChar);
        }
        return text;
    }
    ...
    private static String machineId(int machineLen) {
        String hostName = BaseMachineUtil.getHostName();
        if (StringUtils.isBlank(hostName)) {
            return StringUtils.repeat("0", machineLen);
        } else {
            return BaseSecurityUtil.md5(true, hostName).substring(0, machineLen);
        }
    }
