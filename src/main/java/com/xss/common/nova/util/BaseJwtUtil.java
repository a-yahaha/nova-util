...
public class BaseJwtUtil {
    ...
    public static JwtToken getToken(Object content, Date validTime, long expireInSecond, String secret) {
        ...
        if(content != null) {
            if(content instanceof Collection) {
                claims.put(CONTENT, BaseJsonUtil.readValues(BaseJsonUtil.writeValue(content), Map.class));
            } else {
                claims.put(CONTENT, BaseJsonUtil.readValue(BaseJsonUtil.writeValue(content), Map.class));
            }
        }
    ...
    public static <T> T verifyToken(String jwtToken, String secret, Class<T> resultCls) {
        jwtToken = BaseStringUtil.substringAfter(jwtToken, " ").trim();
        ...
        try {
            ...
            if (resultCls == Map.class) {
                ...
                return (T) BaseJsonUtil.readValue((Map) claims.get(CONTENT), resultCls);
            }
        } catch (Throwable e) {
    ...
    public static Map<String, Object> getTokenInfo(String jwtToken) {
        ...
        try {
            ...
            return BaseJsonUtil.readValue(BaseSecurityUtil.base64Decode(contentPart), Map.class);
        } catch (Throwable e) {
