...
public class BaseIdCardUtil {
    ...
    static {
        Properties prop = BasePropertiesUtil.load("district-info.properties");
        prop.entrySet().forEach(entry ->
                districtInfoMap.put((String) entry.getKey(), BaseJsonUtil.readValue((String) entry.getValue(), DistrictInfo.class))
        );
    }
    ...
    public static IdCardInfo parseIdCard(String idCard, Boolean check) {
        ...
        if (StringUtils.isNumeric(birthDate)) {
            try {
                ...
                info.setBirthDate(BaseDateUtil.toDateFormat(date));
                info.setAge(BaseDateUtil.compareYear(new Date(), date));
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String s : BaseFileUtil.fileToString("test.txt").split(",")) {
            String[] arr = s.split(":");
            String code = arr[0];
            if (!districtInfoMap.containsKey(code)) {
...
//        IOUtils.readLines(BaseIdCardUtil.class.getClassLoader().getResourceAsStream("zfq02"), Charsets.UTF_8).forEach(line -> {
