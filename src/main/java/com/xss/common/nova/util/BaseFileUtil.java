...
public class BaseFileUtil {
    public static DirInfo getDirInfo(String dir, String fileType) throws Exception {
        ...
        String rootPath = URLDecoder.decode(BaseFileUtil.class.getClassLoader().getResource(dir).getPath(), "UTF-8");
    ...
    public static String fileToString(File file, Charset charset) {
        try {
            if (!file.isFile()) { //如果file为jar包中的文件时
                ...
                List<String> lines = IOUtils.readLines(BaseFileUtil.class.getClassLoader().getResourceAsStream(path), charset);
    ...
    private static File nameToFile(String name) throws Exception {
        URL url = BaseFileUtil.class.getClassLoader().getResource(name);
        if (url == null) {
